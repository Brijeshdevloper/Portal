...
//  readonly APIURL = 'http://103.76.214.46/Automate/SmartBranch/API';
  HoUrl='';
  APIURL = '';
  PhotoURL = '';
  Branchid='';
  ...
  constructor(private http: HttpClient, appConfig: AppConfig) {
    ...
    this.HoUrl = appConfig.HoUrl;
    this.Branchid=appConfig.branchID;
  }

  GetApiUrl(val: any) {
    return this.http.post<any>(this.HoUrl + '/BranchApiUrl', val);
  }
...
GetTokenDetail(val:any){
  return this.http.post<any>(this.HoUrl + '/TokenId/GetTokenDetail', val);
}
  GetTokenId(val: any) {
